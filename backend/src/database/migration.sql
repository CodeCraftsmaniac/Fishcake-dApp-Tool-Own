...
DROP POLICY IF EXISTS "Allow public read for config" ON mining_config;
DROP POLICY IF EXISTS "Allow public read for scheduler_state" ON scheduler_state;
DROP POLICY IF EXISTS "Block anonymous inserts on wallets" ON mining_wallets;
DROP POLICY IF EXISTS "Block anonymous updates on wallets" ON mining_wallets;
DROP POLICY IF EXISTS "Block anonymous deletes on wallets" ON mining_wallets;
...
DROP TRIGGER IF EXISTS update_wallets_updated_at ON mining_wallets;
CREATE TRIGGER update_wallets_updated_at
  BEFORE UPDATE ON mining_wallets
  FOR EACH ROW
  EXECUTE FUNCTION update_updated_at();

DROP TRIGGER IF EXISTS update_config_updated_at ON mining_config;
CREATE TRIGGER update_config_updated_at
  BEFORE UPDATE ON mining_config
  FOR EACH ROW
  EXECUTE FUNCTION update_updated_at();

DROP TRIGGER IF EXISTS update_scheduler_updated_at ON scheduler_state;
CREATE TRIGGER update_scheduler_updated_at
  BEFORE UPDATE ON scheduler_state
  FOR EACH ROW
  EXECUTE FUNCTION update_updated_at();

DROP TRIGGER IF EXISTS update_events_updated_at ON mining_events;
